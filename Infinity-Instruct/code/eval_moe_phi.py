...
def forward_with_expert_masks(self, *args, expert_masks=None, **kwargs):
    ...
    for idx, layer in enumerate(self.model.layers):
        if hasattr(layer, 'block_sparse_moe'):
            ...
            layer.block_sparse_moe.current_expert_mask = expert_masks[
                moe_layer_idx] if expert_masks is not None else None
...
if __name__ == "__main__":
    ...
    indices = sorted(range(len(data_inputs)),
                     key=lambda k: len(data_inputs[k].split(" ")))
    ...
    num_layers = len(
        [layer for layer in bc_model.model.layers if hasattr(layer, 'block_sparse_moe')])
    expert_masks = []
    ...
    for i in tqdm(range(0, len(data_inputs), BATCH_SIZE)):
        ...
        combined_input_ids = torch.nn.utils.rnn.pad_sequence(
            combined_input_ids, batch_first=True)
        ...
        with torch.no_grad():
            ...
            for idx, target_start_idx in enumerate(input_lengths):
                ...
                target_ids = inputs["input_ids"][idx:idx+1, target_start_idx:]
                target_attention_mask = attention_mask[idx:idx +
                                                       1, target_start_idx:]
                # 创建标签，并将填充位置设置为-100
    ...
    print(
        f"Contextual Perplexity (excluding inf/nan): {contextual_perplexity}")
    ...
    csv_file = './results/results_phi_moe.csv'
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(
            columns=['mask', 'domain', 'mean_perplexity', "median_perplexity"])
    ...
    df_normal.to_parquet(
        f'./output/phi_{domain}_{args.output_prefix}_normal.parquet')
    df_abnormal.to_parquet(
        f'./output/phi_{domain}_{args.output_prefix}_abnormal.parquet')
